...
export function CookingMode({
  ...
  >
      ...
      <div className="flex border-b border-border bg-card">
        <button onClick={() => setCurrentTab('ingredients')} className={cn("flex-1 py-3 px-4 text-sm font-medium transition-colors relative", currentTab === 'ingredients' ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
          <span className="flex items-center justify-center gap-2">
            <Utensils className="w-4 h-4" />
            Ingredients
            {allIngredientsChecked && <Check className="w-4 h-4 text-green-500" />}
          </span>
          {currentTab === 'ingredients' && <motion.div layoutId="cooking-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary" />}
        </button>
        <button onClick={() => setCurrentTab('steps')} className={cn("flex-1 py-3 px-4 text-sm font-medium transition-colors relative", currentTab === 'steps' ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
          <span className="flex items-center justify-center gap-2">
            <ChefHat className="w-4 h-4" />
            Recipe
          </span>
          {currentTab === 'steps' && <motion.div layoutId="cooking-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary" />}
        </button>
      </div>
      ...
      <main className="flex-1 overflow-auto">
        <AnimatePresence mode="wait">
          {currentTab === 'ingredients' ? <motion.div key="ingredients" initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          ...
          x: -20
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {checkedIngredients.size > 0 && <Button variant="ghost" size="sm" onClick={() => setCheckedIngredients(new Set())} className="text-xs h-7 px-2">
                    Reset all
                  </Button>}
              </div>
            ...
            return <motion.button key={item.id} onClick={() => toggleIngredient(item.id)} className={cn("w-full flex items-center gap-3 py-2.5 px-3 rounded-lg transition-all text-left", isChecked ? "bg-primary/10 border border-primary/30" : "bg-card border border-transparent shadow-soft")} whileTap={{
              scale: 0.98
            }}>
                    ...
                    <span className={cn("text-xs text-muted-foreground flex-shrink-0", isChecked && "line-through")}>
                      {item.quantity} {item.unit}
                    </span>
                  </motion.button>;
          })}
            </motion.div> : <motion.div key="steps" initial={{
          opacity: 0,
          x: 20
        }} animate={{
          opacity: 1,
          ...
          x: 20
              ...
              {recipe.description ? <div className="space-y-4">
                  ...
                  <div className="bg-secondary/50 rounded-xl p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {recipe.items.map(item => <span key={item.id} className={cn("text-xs px-2 py-1 rounded-full bg-background", checkedIngredients.has(item.id) && "line-through opacity-60")}>
